...
export const ThreeLogo = () => {
  ...
  useEffect(() => {
    ...
    camera.position.set(0, isMobile ? 0.45 : 0.7, isMobile ? 5.5 : 5.7);
    ...
    const softboxL = new THREE.RectAreaLight(0xcccccc, 60, 5.2, 3.4);
    softboxL.position.set(-20.4, 2.8, 16.0);
    softboxL.lookAt(0, 1.0, 0);
    scene.add(softboxL);

    const softboxR = new THREE.RectAreaLight(0xcccccc, 60, 5.2, 3.4);
    softboxR.position.set(20.4, 2.8, 16.0);
    softboxR.lookAt(0, 1.0, 0);
    scene.add(softboxR);

    const logoGroup = new THREE.Group();
    logoGroup.position.set(0, isMobile ? 0.7 : 0.85, 0.25);
    scene.add(logoGroup);

    const START_SCALE = isMobile ?  0.13 : 0.14;
    const TARGET_SCALE = isMobile ?  0.72 : 0.88;
    ...
    const BASE_SPEED = isMobile ? 0.008 : 0.012;
    ...
    const onScroll = () => {
      ...
      if (isMobile) {
        logoGroup.position.y = THREE.MathUtils.lerp(0.7, 0.9, p);
        scrollScale = THREE.MathUtils.lerp(1, 0.78, p);
      } else {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        logo.position.y += isMobile ? 0.14 : 0.2;
        logo.scale.set(isMobile ? 8.4 : 9, isMobile ? 8.4 : 9, isMobile ? 6.6 : 7);
    ...
    const animate = () => {
      ...
      logoGroup.rotation.x = isMobile ? 0 : smoothY * 0.22;
      logoGroup.rotation.z = isMobile ? 0 : -smoothX * 0.22;
