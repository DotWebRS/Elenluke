...
export default function Snow({ className }: Props) {
  ...
  useEffect(() => {
    ...
    const SCALE = 0.55; // lower = faster
    const STEP = 2;     // 2 = nicer, 3 = faster
    ...
    const tick = () => {
      ...
      for (let y = yy; y < h; y += STEP) {
        for (let x = xx; x < w; x += STEP) {
          ...
          const v = Math.min(170, Math.max(0, n)); // grayscale dark

          data[idx] = v;
          data[idx + 1] = v;
          data[idx + 2] = v;
          data[idx + 3] = 150; // visibility of dots
        }
      }

      ctx.putImageData(imageData, 0, 0);

      // Vertical scan lines (more visible)
      if ((framePos % 6) === 0) {
        const lines = 2 + ((Math.random() * 3) | 0); // 2–4
