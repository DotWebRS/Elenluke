...
public class ChatController : ControllerBase
{
    ...
    private async Task<bool> CanAccessSubmission(Guid submissionId)
    {
        ...
        if (role == "PortalUser")
        {
            ...
            return await _db.SubmissionParticipants.AsNoTracking().AnyAsync(x => x.SubmissionId == submissionId && x.UserId == uid.Value);
        }
    ...
    public async Task<IActionResult> MyChats()
    {
        ...
        if (role == "PortalUser")
        {
            ...
            var allowed = _db.SubmissionParticipants.AsNoTracking().Where(x => x.UserId == uid.Value).Select(x => x.SubmissionId);
    ...
    public async Task<IActionResult> GetMessages(Guid submissionId)
    {
        ...
        var messages = await _db.SubmissionMessages.AsNoTracking()
            ...
            .Select(x => new { x.Id, x.SubmissionId, x.SenderType, x.SenderEmail, x.Body, x.IsInternal, x.CreatedAtUtc })
            .ToListAsync();
    ...
    public async Task<IActionResult> AddParticipant(Guid submissionId, [FromBody] AddParticipantRequest request)
    {
        ...
        await _hub.Clients.Group($"user:{user.Id}").SendAsync("submission_assigned", new { submissionId, submission.Name, submission.Email });
