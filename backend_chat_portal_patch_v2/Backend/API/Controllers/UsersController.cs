...
public class UsersController : ControllerBase
{
    ...
    public IActionResult Create([FromBody] CreateUserRequest request)
    {
        ...
        if (role != "Admin" && role != "Editor" && role != "Inbox" && role != "PortalUser")
            return BadRequest("Role must be Admin, Editor, Inbox, or PortalUser.");
    ...
    public IActionResult Update([FromRoute] Guid id, [FromBody] UpdateUserRequest request)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Role))
        {
            ...
            if (nextRole != "Admin" && nextRole != "Editor" && nextRole != "Inbox" && nextRole != "PortalUser")
                return BadRequest("Role must be Admin, Editor, Inbox, or PortalUser.");
